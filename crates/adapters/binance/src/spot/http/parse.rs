...
mod tests {
    ...
    /// accepts the current vendor encoding while remaining version-tolerant.
    #[rstest]
    fn test_decode_real_captured_exchange_info_schema_v5() {
        ...
        assert_eq!(schema_id, SBE_SCHEMA_ID);
        assert_eq!(version, 5);

        let info = decode_exchange_info(buf).expect(
            "captured schema-3:5 exchangeInfo payload must decode after version-tolerant SBE fix",
        );
        assert!(
        ...
        assert!(
            info.symbols.iter().any(|s| s.symbol == "BTCUSDT"),

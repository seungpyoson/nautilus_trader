...
use super::{
    ...
    types::{
        ...
        MessageWriter, PingHandler, WriterCommand,
    },
};
#[cfg(feature = "turmoil")]
...
impl WebSocketClientInner {
    ...
    async fn connect_url_with_handler(
        ...
        state_sink: Option<SocketStateSink>,
        connection_rate_limit: Option<ConnectionRateLimit>,
        headers_provider: Option<HeadersProvider>,
    ) -> Result<Self, TransportError> {
        install_cryptographic_provider();
        ...
        let reconnect_headers =
            ReconnectHeaders::from_config(config.headers.clone(), headers_provider)?;
...
///
/// Updating stored headers does not affect the active connection or trigger a reconnect.
/// A configured provider generates fresh headers for each attempt and cannot be updated here.
#[derive(Clone)]
pub struct ReconnectHeaders {
    inner: Arc<ConnectionHeaders>,
}

enum ConnectionHeaders {
    Stored(RwLock<Vec<(String, String)>>),
    Generated(HeadersProvider),
}

impl ReconnectHeaders {
    fn new(headers: Vec<(String, String)>) -> Self {
        Self {
            inner: Arc::new(ConnectionHeaders::Stored(RwLock::new(headers))),
        }
    }

    fn from_config(
        headers: Vec<(String, String)>,
        provider: Option<HeadersProvider>,
    ) -> Result<Self, TransportError> {
        match provider {
            Some(provider) => {
                if !headers.is_empty() {
                    return Err(TransportError::Io(std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        "Cannot combine stored WebSocket headers with a headers provider",
                    )));
                }
                Ok(Self {
                    inner: Arc::new(ConnectionHeaders::Generated(provider)),
                })
            }
            None => Ok(Self::new(headers)),
        }
    }
    ...
    /// # Errors
    ///
    /// Returns an error if headers are generated by a provider, the header name or value is invalid,
    /// or the shared state is poisoned.
    pub fn update(&self, name: &str, value: &str) -> Result<(), TransportError> {
        let ConnectionHeaders::Stored(headers) = self.inner.as_ref() else {
            return Err(TransportError::Io(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "Cannot update generated WebSocket headers",
            )));
        };
        ...
        let mut headers = headers.write().map_err(|_| {
    ...
    fn snapshot(&self) -> Result<Vec<(String, String)>, TransportError> {
        match self.inner.as_ref() {
            ConnectionHeaders::Stored(headers) => {
                headers.read().map(|headers| headers.clone()).map_err(|_| {
                    TransportError::Io(std::io::Error::other(
                        "WebSocket reconnect headers lock poisoned",
                    ))
                })
            }
            ConnectionHeaders::Generated(provider) => provider(),
        }
    }
}
...
mod headers_provider_tests {
    ...
    #[rstest]
    fn each_attempt_generates_one_complete_header_set() {
        let attempts = Arc::new(AtomicUsize::new(0));
        let count = Arc::clone(&attempts);
        let provider: HeadersProvider = Arc::new(move || {
            let generation = count.fetch_add(1, Ordering::SeqCst).to_string();
            Ok(vec![
                ("timestamp".to_string(), generation.clone()),
                ("signature".to_string(), generation),
            ])
        });
        let headers = ReconnectHeaders::from_config(Vec::new(), Some(provider)).unwrap();
        assert_eq!(attempts.load(Ordering::SeqCst), 0);

        for expected in ["0", "1", "2"] {
            let actual = headers.clone().snapshot().unwrap();
            assert_eq!(actual[0].1, expected);
            assert_eq!(actual[1].1, expected);
        }
        assert!(headers.update("timestamp", "stale").is_err());
        assert!(!format!("{headers:?}").contains("signature"));
    }

    #[rstest]
    fn failed_generation_does_not_reuse_previous_headers() {
        let attempts = AtomicUsize::new(0);
        let provider: HeadersProvider =
            Arc::new(move || match attempts.fetch_add(1, Ordering::SeqCst) {
                0 => Ok(vec![("authorization".to_string(), "first".to_string())]),
                _ => Err(TransportError::Other("signing unavailable".to_string())),
            });
        let headers = ReconnectHeaders::from_config(Vec::new(), Some(provider)).unwrap();
        assert_eq!(headers.snapshot().unwrap()[0].1, "first");
        assert!(headers.snapshot().is_err());
    }

    #[rstest]
    fn stored_headers_and_provider_cannot_conflict() {
        let provider: HeadersProvider =
            Arc::new(|| panic!("provider must not run for invalid config"));
        assert!(
            ReconnectHeaders::from_config(
                vec![("authorization".to_string(), "stored".to_string())],
                Some(provider),
            )
            .is_err()
        );
        let stored = ReconnectHeaders::new(vec![("authorization".to_string(), "old".to_string())]);
        stored.update("Authorization", "new").unwrap();
        assert_eq!(
            stored.snapshot().unwrap(),
    ...
    #[rstest]
    #[tokio::test]
    async fn public_builder_propagates_provider_failure_before_network_io() {
        let config = WebSocketConfig::builder()
            .url("wss://example.test".to_string())
            .build()
            .unwrap();
        let provider: HeadersProvider =
            Arc::new(|| Err(TransportError::Other("signing unavailable".to_string())));
        let result = WebSocketClient::epoch_builder()
            .config(config)
            .epoch_handler(Arc::new(|_, _| {}))
            .headers_provider(provider)
            .connect()
            .await;
        assert!(
            matches!(result, Err(TransportError::Other(message)) if message == "signing unavailable")
        );
    }

    #[rstest]
    #[tokio::test]
    async fn initial_connection_quota_wait_precedes_header_generation() {
        let key = Ustr::from("test-connection");
        let limiter = Arc::new(RateLimiter::new_with_quota(
            None,
            vec![(key, Quota::with_period(Duration::from_secs(5)).unwrap())],
        ));
        ...
        let provider: HeadersProvider = Arc::new(move || {
            ...
            Err(TransportError::Other("signing unavailable".to_string()))
        });
        let config = WebSocketConfig::builder()
            .url("wss://example.test".to_string())
            .build()
            .unwrap();
        let result = dst::time::timeout(
            Duration::from_millis(50),
            WebSocketClient::epoch_builder()
                .config(config)
                .epoch_handler(Arc::new(|_, _| {}))
                .connection_rate_limiter(limiter)
                .connection_rate_keys(Arc::from([key]))
                .headers_provider(provider)
                .connect(),
        )
        .await;
...
impl WebSocketClient {
    ...
    /// `headers_provider` generates fresh headers after each connection quota wait and is mutually
    /// exclusive with `config.headers`.
    ///
    ...
    /// - A headers provider is combined with stored headers or fails to generate headers.
    ...
    /// `headers_provider` generates the complete header set for each connection attempt, as in
    /// [`Self::builder`].
    ...
    /// Waits until the transport enters a terminal disconnecting or closed state.
    ///
    /// Reconnectable connection loss does not complete this wait. Consumers can select it
    /// alongside their message receiver so exhausted reconnect attempts cannot leave them waiting
    /// on a channel whose sender is still retained by the client.
    pub async fn wait_until_closed(&self) {
        loop {
            ...
            tokio::select! {
                biased;
                () = notified => {}
                () = dst::time::sleep(Duration::from_millis(100)) => {}
            }
        }
    }
    ...
    async fn await_rate_limit_or_closed(&self, keys: Option<&[Ustr]>) -> Result<(), SendError> {
        tokio::select! {
            ...
            () = self.wait_until_closed() => Err(SendError::Closed),
        }
    }

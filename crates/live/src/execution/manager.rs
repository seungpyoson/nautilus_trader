...
/// Native accounting orders which close a resolved contract without changing venue inventory.
static TAG_CONTRACT_SETTLEMENT: LazyLock<Ustr> =
    LazyLock::new(|| Ustr::from("CONTRACT_SETTLEMENT"));
...
impl ExecutionManager {
    ...
    pub(crate) fn prepare_contract_settlements(
        ...
        for position in positions {
            ...
            let order = MarketOrder::new_checked(
                ...
                None,
                None,
                Some(vec![*TAG_CONTRACT_SETTLEMENT]),
            )?
            .into();
    ...
    pub(crate) fn reconcile_execution_mass_status_ref(
        ...
        let position_count: usize = mass_status
            .position_reports_ref()
            .values()
            .map(Vec::len)
            .sum();
        ...
        for event in &events {
            ...
            if let OrderEventAny::Filled(fill) = event
                && let Some(fill_key) = event_fill_keys.get(&fill.event_id).copied()
            {
    ...
    fn position_report_is_reconciled(&self, cache: &Cache, report: &PositionStatusReport) -> bool {
        ...
        let Some(close) = cache
            .instrument_close(&report.instrument_id)
            .filter(|close| close.close_type == InstrumentCloseType::ContractExpired)
        else {
            return self.position_quantity_matches(report, cached_qty);
        };
        let Some(settled_qty) = Self::settlement_inventory_offset(cache, report, close.close_price)
        else {
            return false;
        };

        // Venue inventory may remain held after the accounting close or be fully redeemed.
        // A partial or unexplained holding stays unresolved; neither branch creates a fill.
        (cached_qty.is_zero() && report.signed_decimal_qty.is_zero())
            || self.position_quantity_matches(report, cached_qty + settled_qty)
    }

    fn settlement_inventory_offset(
        cache: &Cache,
        report: &PositionStatusReport,
        close_price: Price,
    ) -> Option<Decimal> {
        let mut quantity = Decimal::ZERO;
        for id in cache.iter_client_order_ids(None, Some(&report.instrument_id), None, None) {
            let order = cache.order_ref(&id)?;
            if !order
                .tags()
                .is_some_and(|tags| tags.contains(&*TAG_CONTRACT_SETTLEMENT))
                || order
                    .account_id()
            ...
            let position_id = cache.position_id(&id)?;
            let position = cache.position_ref(position_id)?;
            if position.account_id != report.account_id
                || report
                    .venue_position_id
            ...
            if position.instrument_id != report.instrument_id
                || order.account_id() != Some(report.account_id)
                || order.status() != OrderStatus::Filled
                || !order.is_reduce_only()
                || order.filled_qty() != order.quantity()
                || order.avg_px() != Some(close_price.as_decimal())
            {
                return None;
            }
    ...
    pub(crate) fn check_mass_status_inventory(
        ...
        for id in cache.iter_position_open_ids(None, None, None, Some(&mass_status.account_id)) {
            ...
            summary.source_valid &= client.handles_order_venue(position.instrument_id.venue)
                ...
                && !cache
                    .instrument_close(&position.instrument_id)
                    .is_some_and(|close| close.close_type == InstrumentCloseType::ContractExpired);
    ...
    fn position_quantity_postcondition(
        ...
        if self.has_contract_close(&key.0) {
            ...
            return cache
                ...
                && reports
                    .iter()
                    .all(|report| self.position_report_is_reconciled(&cache, report));
        }
        self.position_quantity_comparison(key, reports)
            .quantities_match(self.position_reconciliation_tolerance(key.1))
    }

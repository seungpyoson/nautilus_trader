...
impl ExecutionManager {
    ...
    fn settlement_inventory_offset(
        ...
        for position_id in cache.iter_position_ids(
            None,
            Some(&report.instrument_id),
            None,
            Some(&report.account_id),
        ) {
            if report.venue_position_id.is_some_and(|id| id != position_id) {
                continue;
            }
            ...
            for event in &position.replay_events {
                ...
                if !fill.is_contract_settlement() {
                    continue;
                }
                ...
                if fill.account_id != report.account_id
                    ...
                    || fill.last_px != close_price
                    || position.fill_voids.iter().any(|correction| {
        ...
        for id in cache.iter_position_open_ids(None, None, None, Some(&mass_status.account_id)) {
            ...
            summary.source_valid &= client.handles_order_venue(position.instrument_id.venue)
                ...
                && cache
                    ...
                    .is_none_or(|close| close.close_type != InstrumentCloseType::ContractExpired);

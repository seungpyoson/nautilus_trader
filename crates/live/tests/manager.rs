...
#[rstest]
#[tokio::test]
async fn test_check_open_orders_targeted_query_selects_most_advanced_valid_report() {
    let config = ExecutionManagerConfig {
        open_check_threshold_ns: 0,
        open_check_missing_retries: 1,
        open_check_open_only: false,
    ...
    let client_order_id = ClientOrderId::from("O-TARGETED-ADVANCED");
    let venue_order_id = VenueOrderId::from("V-TARGETED-ADVANCED");
    let order = OrderTestBuilder::new(OrderType::Limit)
        .client_order_id(client_order_id)
        .instrument_id(test_instrument_id())
        .quantity(Quantity::from("10.0"))
    ...
    .with_price(Price::from("100.0"));
    let terminal_report = create_order_status_report(
        Some(client_order_id),
        venue_order_id,
        test_instrument_id(),
        OrderStatus::Canceled,
    ...
    let first_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-STALE"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(stale_report);
    let second_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-CURRENT"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(terminal_report);
    let clients: Vec<&dyn ExecutionClient> = vec![&first_client, &second_client];
...
#[rstest]
#[tokio::test]
async fn test_check_open_orders_targeted_query_defers_equal_authority_conflict() {
    let config = ExecutionManagerConfig {
        open_check_threshold_ns: 0,
        open_check_missing_retries: 1,
        open_check_open_only: false,
    ...
    let client_order_id = ClientOrderId::from("O-TARGETED-EQUAL-CONFLICT");
    let venue_order_id = VenueOrderId::from("V-TARGETED-EQUAL-CONFLICT");
    let order = OrderTestBuilder::new(OrderType::Limit)
        .client_order_id(client_order_id)
        .instrument_id(test_instrument_id())
        .quantity(Quantity::from("10.0"))
    ...
    .with_cancel_reason("FIRST".to_string());
    let second_report = create_order_status_report(
        Some(client_order_id),
        venue_order_id,
        test_instrument_id(),
    ...
    .with_cancel_reason("SECOND".to_string());
    let first_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-EQUAL-FIRST"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(first_report);
    let second_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-EQUAL-SECOND"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(second_report);
    let clients: Vec<&dyn ExecutionClient> = vec![&first_client, &second_client];
...
#[rstest]
#[tokio::test]
async fn test_check_open_orders_targeted_query_prefers_current_venue_report() {
    let config = ExecutionManagerConfig {
        open_check_threshold_ns: 0,
        open_check_missing_retries: 1,
        open_check_open_only: false,
    ...
    let client_order_id = ClientOrderId::from("O-TARGETED-CURRENT-VENUE");
    let historical_venue_order_id = VenueOrderId::from("V-TARGETED-HISTORICAL");
    ...
    let order = ctx.cache.borrow_mut().update_order(&submitted).unwrap();
    let accepted =
        TestOrderEventStubs::accepted(&order, test_account_id(), historical_venue_order_id);
    let order = ctx.cache.borrow_mut().update_order(&accepted).unwrap();
    ...
    let first_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-HISTORICAL"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(historical_report);
    let second_client = MockExecutionClient::for_venue(
        ClientId::from("CLIENT-TARGETED-CURRENT"),
        test_venue(),
        Vec::new(),
    )
    .with_order_report(current_report);
    let clients: Vec<&dyn ExecutionClient> = vec![&first_client, &second_client];
...
#[rstest]
#[tokio::test]
async fn test_check_open_orders_targeted_query_defers_stale_negative_evidence() {
    let config = ExecutionManagerConfig {
        open_check_threshold_ns: 0,
        open_check_missing_retries: 1,
        open_check_open_only: false,
    ...
    let client_order_id = ClientOrderId::from("O-TARGETED-STALE-NONE");
    let original_venue_order_id = VenueOrderId::from("V-TARGETED-STALE-NONE-ORIGINAL");
    let current_venue_order_id = VenueOrderId::from("V-TARGETED-STALE-NONE-CURRENT");
    insert_accepted_limit_order(
        &ctx,
        client_order_id,
        original_venue_order_id,
    ...
    let cache = ctx.cache.clone();
    let mock_client =
        MockExecutionClient::new(Vec::new()).with_on_order_report_query(Box::new(move || {
            cache.borrow_mut().update_order(&updated).unwrap();
        }));
    let clients: Vec<&dyn ExecutionClient> = vec![&mock_client];
    ...
    assert_eq!(ctx.manager.recon_check_retry_count(&client_order_id), 1);
    assert_eq!(
        ctx.get_order(&client_order_id).unwrap().venue_order_id(),

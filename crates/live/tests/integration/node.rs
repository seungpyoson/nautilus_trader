...
impl DataActor for ReconciliationSummaryActor {
    fn on_start(&mut self) -> anyhow::Result<()> {
        *self.observed.lock() = self.handle.startup_reconciliation_summary();
        self.handle.stop();
        Ok(())
    }
}
...
impl DataActor for StartupCacheActor {
    fn on_start(&mut self) -> anyhow::Result<()> {
        ...
        *self.observed.lock() = Some(observation);
        self.handle.stop();
        Ok(())
    }
}
...
pub(crate) mod serial_tests {
    ...
    impl ExecutionClient for StartupMassStatusExecutionClient {
        ...
        async fn connect(&mut self) -> anyhow::Result<()> {
            self.state.connected.store(true, Ordering::Relaxed);
            for event in self.state.connect_system_events.lock().drain(..) {
        ...
        async fn generate_mass_status(
            ...
            for event in self.state.queued_exec_events.lock().drain(..) {
                get_exec_event_sender().send(event).unwrap();
            }

            for event in self.state.queued_system_events.lock().drain(..) {
                get_system_event_sender().send(event).unwrap();
            }

            match self.behavior {
                StartupMassStatusBehavior::Available => {
                    let mut report = self.state.mass_status.lock();
    ...
    #[rstest]
    #[case::held("10.00", true)]
    #[case::redeemed("0.00", true)]
    #[case::excess("11.00", false)]
    #[case::partial_unexplained("5.00", false)]
    #[tokio::test(flavor = "current_thread")]
    async fn test_startup_settled_inventory_uses_native_closes(
        ...
        use nautilus_model::{
            ...
            enums::{InstrumentCloseType, PositionSide},
        ...
        let position_id = PositionId::from("SETTLEMENT-OWNER");
        let state = StartupMassStatusClientState::default();
        let config = LiveNodeConfig {
            exec_engine: LiveExecutionEngineConfig {
                reconciliation: true,
        ...
        let mut node = LiveNodeBuilder::from_config(config)
            .unwrap()
            .with_name("StartupSettlementInventory")
            .add_exec_client(
        ...
        let mut order = OrderTestBuilder::new(OrderType::Limit)
            ...
            .instrument_id(instrument_id)
            .client_order_id(ClientOrderId::from("OPEN-SETTLEMENT"))
            .side(OrderSide::Buy)
        ...
        order
            .apply(TestOrderEventStubs::accepted(
                &order,
                account_id,
                VenueOrderId::from("VENUE-OPEN-SETTLEMENT"),
            ))
            .unwrap();
        let OrderEventAny::Filled(fill) = TestOrderEventStubs::filled(
            &order,
            &instrument,
            Some(TradeId::from("OPEN-SETTLEMENT")),
            Some(position_id),
        ...
        let quantity = Quantity::from(reported_quantity);
        report.add_position_reports(vec![PositionStatusReport::new(
            account_id,
            instrument_id,
            if quantity.is_zero() {
                PositionSide::Flat
        ...
        *state.mass_status.lock() = Some(report);
        ...
        let summary = observed
            .lock()
            .clone()
            .expect("trader observed the startup summary");
        let collected = summary.clients[0].report.as_ref().unwrap();
        assert!(collected.has_complete_account_collection(None));
        assert_eq!(
            collected.application.all_received_reports_reconciled(),
            expected_reconciled
        );
        assert_eq!(
            collected
        ...
        assert_eq!(summary.all_clients_reconciled(), expected_reconciled);
        let cache = node.kernel().cache();
        let cache = cache.borrow();
        let position = cache.position_ref(&position_id).unwrap();
        ...
        *state.mass_status.lock() = Some(report);
        ...
        if incomplete_ingress {
            let missing = OrderTestBuilder::new(OrderType::Market)
                ...
                .build();
            state.queued_exec_events.lock().push(ExecutionEvent::Order(
                TestOrderEventStubs::accepted(
                    &missing,
    ...
    async fn test_startup_final_inventory_checks_cached_only_orders(
        ...
        *state.mass_status.lock() = Some(ExecutionMassStatus::new(
            client_id,
            account_id,
            instrument_id.venue,
            UnixNanos::from(1),
    ...
    async fn test_startup_final_inventory_rechecks_fill_only_closed_order_sources(
        ...
        *state.mass_status.lock() = Some(mass_status);
        *state.queued_exec_events.lock() = vec![
    ...
    async fn test_execution_callback_aborts_before_actor_start(
        ...
        *state.queued_exec_events.lock() = vec![
        ...
        let raw_handler = ShareableMessageHandler::from_typed({
            ...
            move |_report: &OrderStatusReport| {
                *during_reconciliation.lock() = Some((
                    handle.startup_reconciliation_summary().is_none(),
                    clock.borrow().timestamp_ns(),
                ));
                handle.stop();
        ...
        if stop_before_pending {
            let (unpublished, ts_reconciliation) = during_reconciliation.lock().unwrap();
            assert!(
                unpublished,
            ...
            if let Some((unpublished, _)) = *during_dispatch.lock() {
                assert!(
                    !unpublished,
            ...
            let (unpublished, ts_dispatch) = during_dispatch.lock().unwrap();
            assert!(
                unpublished,
                "summary must remain unavailable during pending dispatch"
            );
    ...
    async fn test_startup_keeps_socket_notifications_deferred_and_ordered(#[case] run: bool) {
        ...
        let handler = ShareableMessageHandler::from_typed({
            ...
            move |event: &SocketStateChanged| {
                notifications
                    .lock()
                    .push((event.state, observed.lock().is_some()));
            }
        });
        ...
        *state.mass_status.lock() = Some(report);
        ...
        if enabled {
            ...
            if matches!(behavior, StartupMassStatusBehavior::Available) {
                ...
                assert_eq!(
                    received.ts_init,
                    state.mass_status.lock().as_ref().unwrap().ts_init
                );
                assert!(

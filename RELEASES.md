...
### Fixes
- Fixed Polymarket reconciliation silently understating filled quantity when a confirmed maker trade holds none of the account's own maker orders; no fill report can be built from such a trade, so it is now counted and reported to the operator rather than skipped with no counter and no log entry
- Fixed Polymarket maker-order ownership comparing the configured account address case-sensitively, so a checksummed funder failed the address test for every one of the account's own maker orders
